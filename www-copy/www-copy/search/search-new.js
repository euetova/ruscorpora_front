var settings = {
    ...
    addBrotherImage : '<img title="Добавить слово" alt="Добавить слово" src="i/add_sibling.gif" border="0" width="18" height="18" />',
    addSonImage : '<img title="Добавить потомка" alt="Добавить потомка" src="i/add_son.gif" border="0" width="18" height="18" />',
    killImage : '<img title="Удалить слово" alt="Удалить слово" src="i/kill.gif" border="0" width="18" height="18" />',
    defaultMin : '1',
    defaultMax : '1',
    defaultType : '',
    disableTree : false,
...
function draw_word ( block, id, parentid, level ) {
    ...
    if (block == 0) {
        if ( no_words == 0 ) {
            innercode +=
                '<table class="distreq"><tr><td>Расстояние' +  (settings.disableTree?'':' от родителя') + ':'+
                    ' от <input type="text" id="' + get_id(block, "min") + id + '" name="min' + id + '" size="4" value="' + settings.defaultMin + '" class="text" />'+
                    ' до <input type="text" id="' + get_id(block, "max") + id + '" name="max' + id + '" size="4" value="' + settings.defaultMax + '" class="text" />'+
            ...
            if ( settings.linksName ) {
                innercode +=
                    ...
                    'Синтаксическое отношение <input type="text" id="' + get_id(block, "type") + id + '" name="type' + id + '" value="' + settings.defaultType + '" class="text" size="60" /> ' +
                    ' <a href="' + settings.linksHelp + '" onClick="return showhelp(\'' + settings.linksHelp + '\',0.7)">' + settings.helpImage + '</a> '+
                    '<a href="javascript:getLinks(' + id + ')">выбрать</a>' +
                    '</td></tr>';
            }
            innercode += '</table>';
        }
            ...
            '<table class="wordreq">' +
                '<tr valign="bottom">' +
                    '<td class="' + widthSettings + '">' +
                        ...
                        ' <a href="#" onClick="return getVkb(\'lex' + id + '\' )"><img style="vertical-align:bottom;" src="i/keyboard.gif" alt="Виртуальная клавиатура" title="Виртуальная клавиатура" width="42" height="21" border="0"></a>' +
                    '</td>' +
                    '<td class="' + widthSettings + '">' +
                        'грамм. признаки ' +
                        ' <a href="' + settings.grammHelp + '" onClick="return showhelp(\'' + settings.grammHelp + '\',0.8)">' + settings.helpImage + '</a>'+
                        ' <a href="javascript:getGramm(' + id + ')">выбрать</a>'+
                    '</td>';
        if (settings.semName) {
            innercode +=
                    '<td class="' + widthSettings + '">' +
                        ...
                        ' <a href="javascript:getSem(' + id + ')">выбрать</a>&nbsp;'+
                    '</td>';
        }
        innercode +=
                    '<td>&nbsp;</td>' +
            ...
            innercode +=
                '<tr>'+
                    '<td class="wordreqtext" style="padding:0 10px 10px 10px">'+
                        ...
                        ' <a href="javascript:getFlags(' + id + ')">выбрать</a>'+
                    '</td>'+
                    '<td class="wordreqinput">'+
            ...
            '<table class="wordreq">' +
                '<tr valign="bottom">' +
                    '<td class="' + widthSettings + '">' +
                        ...
                        ' <a href="javascript:getAttr(' + id + ')">выбрать</a>'+
                    '</td>' +
                    '<td class="' + widthSettings + '"/>';
        innercode +=
                    '<td>&nbsp;</td>' +
